...
credit_number <- as.numeric(args)
#loan_id <- as.numeric(strsplit(args,"_")[[1]][1])
#report_id <- as.numeric(strsplit(args,"_")[[1]][2])
#loan_id <- 233674
product_id <- 1

loan_id<-as.numeric(dbFetch(dbSendQuery(con,paste(
  "SELECT id AS loan_id 
  FROM ",db_name,".loans 
  WHERE credit_number = ",credit_number,";",sep="")),n=-1))
...
final <- scoring_df
other_creds<-readxl::read_xlsx(
            paste(main_dir,"Monitoring\\Files\\Vintage_Scores.xlsx", sep=""))

final[names(other_creds)[which(!names(other_creds)%in%names(final))]]<-NA
other_creds[names(final)[which(!names(final)%in%names(other_creds))]]<-NA

final<-rbind(final,other_creds)
final<-final[which(!duplicated(final$loan_id)),]
